...
// Quantization version when Docid-oriented indexes are used
inline bool DocidOriented_BMW_BMQ::pickCandidate(lptrArray& lps, const float threshold, const int numOfTermsInQ, int& pivot, float& sumOfBLockMaxesUptoPivot) {
	...
	sumOfBLockMaxesUptoPivot = 	lps[pivot]->quantile*(float) lps[pivot]->gen.get_Quantized_Score(lpspivot->did);
	for(int i = 0; i < pivot; ++i) 	{
		sumOfBLockMaxesUptoPivot +=  lps[i]->quantile*(float) lps[i]->gen.get_Quantized_Score(lpspivot->did);
	}
	return true;
}

inline void DocidOriented_BMW_BMQ::handleReal(lptrArray& lps, const int pivot, float sumOfBLockMaxesUptoPivot,PriorityArray<QpResult>& resultsHeap, const float threshold, const int topK) {
	...
	if( did == lps[0]->did ) { // this dude needs to be evaluated!!
		for(int i = 0 ; i < pivot + 1; ++i)	{
			...
			sumOfBLockMaxesUptoPivot -= ( lps[i]->quantile*(float) lps[i]->gen.get_Quantized_Score(did) - scoret );
			//early termination after substr. deltas of real scores from approximated upper bound
			if( !(sumOfBLockMaxesUptoPivot > threshold) ) // > TOCHANGE
				goto mvptrs;
		}
...
inline bool DocidOriented_BMW_BMQ::handleFake(lptrArray& lps, const int pivot, const int numOfTermsInQ) {
	int least_ft = lps[0]->lengthOfList;
	int least_i = 0;
...
PriorityArray<QpResult> DocidOriented_BMW_BMQ::operator()( lptrArray& lps, const int topK) {
		int numOfTermsInQ;
		...
		while(true) {
			const float threshold =resultsHeap.head().score;// res[topK-1].score;
...
PriorityArray<QpResult> DocidOriented_BMW_BMQ::operator()( lptrArray& lps, const int topK, const float& theta) {
		int numOfTermsInQ;
		float sumOfBLockMaxesUptoPivot;
		int pivot,smallestMaxDid;
		lptr* lpspivot;
		...
		//const float threshold = theta;
		while(true) {
			//const float threshold = Fcompare(theta, resultsHeap.head().score) >=0 ? theta : resultsHeap.head().score; //resultsHeap.head().score;// res[topK-1].score;
