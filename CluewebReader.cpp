...
//		inf_prefix_sum[i/4 + 1] = (2*inf_buffer[i+1]*sizeof(unsigned int))+inf_prefix_sum[i/4];
	...
	// inf_buffer[0] = term_id and inf_buffer[1] == padded postings
		...
		termid_ = new unsigned int[47978];
		listLen_ = new unsigned long[47978];
		offset_ = new unsigned long[47978];
		listLenBytes_ = new unsigned long[47978];

		load_doclength();
		cout<<"doclength is loaded"<<endl;

		// string input_source( "/home/qi/Dropbox/WSDM_Index_Script/lexicon_baby" );
		// string input_source( "/home/qi/Dropbox/experiments/lexicon_baby_training_set" );
		string input_source( "/home/qi/Dropbox/experiments/lexicon_baby_712" );
...
void CluewebReader::load_doclength(){
	...
	inputstream.open(input_source.c_str());
...
//		 cout<<"doc_id: "<<tList.doc_ids.back()+uncompressed_list[2*i]<<" ";
		 ...
		 tList.doc_ids.push_back(tList.doc_ids.back()+uncompressed_list[2*i]);
		 else
