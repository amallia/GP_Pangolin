/*
 * DocidOriented_BMM.cpp
 *
 *  Created on: July 2nd, 2014
 *      Author: Qi
 */

#include "DocidOriented_BMM.h"
#include "globals.h"
#include "profiling.h"
#include "utils.h"
#include <vector>

// Note: in order to run BMM or BMM-NLB, just set the boolean variable skip_to_next_live_block_enabled false or true (accordingly) in the operator code

inline bool DocidOriented_BMM::find_smallest_did_in_essential_set(lptrArray& lps, const int& num_of_essential_lists, int& smallest_did) {
...
//    	essential_block_sum += lists_block_max_score.at(i);
    ...
    if (! (Fcompare(essential_block_sum + current_prefix_sum_max_score, threshold) <= comparison)) {
		...
		for (int i = num_of_essential_lists-1; i>=0; --i) {
			lists_block_max_score[i] = lps[i]->gen.getMaxScore(smallest_did);
...
void DocidOriented_BMM::find_new_non_essential_lists(unsigned int& num_essential_lists, unsigned int& num_of_non_essential_lists, std::vector <float>& prefix_sum_max_score, lptrArray& lps, const float& threshold, bool& essential_list_added) {
...
void DocidOriented_BMM::skip_to_next_live_block(lptrArray& lps, const float& threshold, const int& num_essential_lists, const int& check_up_to_list, const int& find_next_live_block_from_list, int& smallest_did, const bool& skip_to_next_live_block_enabled) {
	...
	smallest_max_did += 1;
   	...
   	while (skip_to_next_live_block_enabled) {
   		...
   		for (int i = lps.size()-1; i>=find_next_live_block_from_list; --i)
   			sum_of_block_maxes += lps[i]->gen.getMaxScore(smallest_max_did);
...
void DocidOriented_BMM::operator() (lptrArray& lps, const int& topK, QpResult* res, const float& threshold, const int& threshold_knowledge){
	...
	bool skip_to_next_live_block_enabled = true; // For BMM-NLB set true, for BMM set false
	...
	while(true) {
		...
		// evaluate
		evaluate(lps, res, current_threshold, topK, num_essential_lists, smallest_did, lists_block_max_score, current_prefix_sum_max_score, check_for_new_essential_list, comparison_heapify, comparison, lists_maxscore, skip_to_next_live_block_enabled);
	}
}
