/*
 * DocidOriented_BMM_Layering.h
 *
 *  Created on: July 2nd, 2014
 *      Author: Qi
 */

#ifndef DOCIDORIENTED_BMM_LAYERING_H_
#define DOCIDORIENTED_BMM_LAYERING_H_

#include "PostingOriented_BMW.h"

class DocidOriented_BMM_Layering{
private:
	unsigned int* pages;
	inline void evaluate(lptrArray& lps, QpResult* res, const float& threshold, const int topK, const int& num_of_essential_lists, int& smallest_did, std::vector <float>& lists_block_max_score, const float& current_prefix_sum_max_score, bool& check_for_new_essential_list, const int& comparison_heapify, const int& comparison, std::vector <float>& lists_maxscore, const bool& skip_to_next_live_block_enabled, const int& num_actual_terms) MYNOINLINE;
	...
	inline void skip_to_next_live_block(lptrArray& lps, const float& threshold, const int& num_essential_lists, const int& check_up_to_list, const int& find_next_live_block_from_list, int& smallest_did, const bool& skip_to_next_live_block_enabled, const int& num_actual_terms);
public:
	DocidOriented_BMM_Layering(unsigned int* pgs) : pages(pgs) {}
	void operator()(lptrArray& lps, const int& k, QpResult* res, const float& threshold, const int& threshold_knowledge);
};

#endif /* DOCIDORIENTED_BMM_LAYERING_H_ */
